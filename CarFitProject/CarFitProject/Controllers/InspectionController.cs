...
namespace CarFitProject.Controllers
{
    public class InspectionController : Controller
    {
        ...
        private readonly IInspectionReportPdfService _pdf;

        public InspectionController(CarFitDbContext context, IInspectionReportPdfService pdf)
        {
            _context = context;
            _pdf = pdf;
        }

        // GET /Inspection/Pdf/{id}  (id = CarId, which is the InspectionReport PK).
        // Public when the car has an Active/Available listing; Admins can export any report.
        [HttpGet]
        public async Task<IActionResult> Pdf(int id)
        {
            var report = await _context.InspectionReports
                .AsNoTracking()
                .Include(r => r.Car)
                .FirstOrDefaultAsync(r => r.CarId == id);
            if (report == null) return NotFound();

            var isPublic = await _context.CarListings
                .AnyAsync(l => l.CarId == id && l.Status == ListingStatus.Approved);
            if (!isPublic && !User.IsInRole("Admin")) return NotFound();

            var glossary = await _context.InspectionTermsGlossaries.AsNoTracking().ToListAsync();
            var isArabic = System.Globalization.CultureInfo.CurrentUICulture.TwoLetterISOLanguageName == "ar";

            var bytes = _pdf.Build(report, glossary, isArabic);
            return File(bytes, "application/pdf", $"inspection-{id}.pdf");
        }
