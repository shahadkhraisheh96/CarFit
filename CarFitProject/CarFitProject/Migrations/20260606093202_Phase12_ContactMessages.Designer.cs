// <auto-generated />
using System;
using CarFitProject.Models;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace CarFitProject.Migrations
{
    [DbContext(typeof(CarFitDbContext))]
    [Migration("20260606093202_Phase12_ContactMessages")]
    partial class Phase12_ContactMessages
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "10.0.7")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("CarFitProject.Models.BuyerContactIntent", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("BuyerUserId")
                        .IsRequired()
                        .HasMaxLength(450)
                        .HasColumnType("nvarchar(450)");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime2")
                        .HasDefaultValueSql("(getutcdate())");

                    b.Property<int>("ListingId")
                        .HasColumnType("int");

                    b.Property<string>("SellerUserId")
                        .HasMaxLength(450)
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex(new[] { "CreatedAt" }, "IX_BuyerContactIntents_CreatedAt");

                    b.HasIndex(new[] { "ListingId" }, "IX_BuyerContactIntents_ListingId");

                    b.ToTable("BuyerContactIntents", (string)null);
                });

            modelBuilder.Entity("CarFitProject.Models.Car", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("BodyType")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("body_type");

                    b.Property<string>("EngineSize")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("engine_size");

                    b.Property<string>("ExteriorColor")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("exterior_color");

                    b.Property<string>("ExteriorOptions")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("exterior_options");

                    b.Property<string>("FuelEfficiency")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasColumnName("fuel_efficiency");

                    b.Property<string>("FuelType")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)")
                        .HasColumnName("fuel_type");

                    b.Property<string>("Images")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("images");

                    b.Property<string>("InteriorColor")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("interior_color");

                    b.Property<string>("InteriorOptions")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("interior_options");

                    b.Property<string>("Kilometers")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("kilometers");

                    b.Property<string>("Make")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasColumnName("make");

                    b.Property<string>("Model")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasColumnName("model");

                    b.Property<decimal?>("Price")
                        .HasColumnType("decimal(18, 2)")
                        .HasColumnName("price");

                    b.Property<string>("RegionalSpecs")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("regional_specs");

                    b.Property<int?>("ScrapedId")
                        .HasColumnType("int")
                        .HasColumnName("scraped_id");

                    b.Property<int?>("Seats")
                        .HasColumnType("int")
                        .HasColumnName("seats");

                    b.Property<string>("TechnologyOptions")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("technology_options");

                    b.Property<string>("Transmission")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)")
                        .HasColumnName("transmission");

                    b.Property<string>("Trim")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("trim");

                    b.Property<string>("Type")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)")
                        .HasColumnName("type");

                    b.Property<int>("Year")
                        .HasColumnType("int")
                        .HasColumnName("year");

                    b.HasKey("Id")
                        .HasName("PK__Cars__3213E83F43C12A3A");

                    b.HasIndex(new[] { "Make" }, "IX_Cars_Make");

                    b.HasIndex(new[] { "Transmission", "BodyType", "Year" }, "IX_Cars_Matching");

                    b.HasIndex(new[] { "Model" }, "IX_Cars_Model");

                    b.HasIndex(new[] { "Price" }, "IX_Cars_Price");

                    b.HasIndex(new[] { "Type" }, "IX_Cars_Type");

                    b.ToTable("Cars");
                });

            modelBuilder.Entity("CarFitProject.Models.CarImage", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("CarId")
                        .HasColumnType("int")
                        .HasColumnName("car_id");

                    b.Property<bool>("IsPrimary")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false)
                        .HasColumnName("is_primary");

                    b.Property<int>("SortOrder")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(0)
                        .HasColumnName("sort_order");

                    b.Property<string>("Url")
                        .IsRequired()
                        .HasMaxLength(2048)
                        .HasColumnType("nvarchar(2048)")
                        .HasColumnName("url");

                    b.HasKey("Id");

                    b.HasIndex(new[] { "CarId" }, "IX_CarImages_car_id");

                    b.ToTable("car_images", (string)null);
                });

            modelBuilder.Entity("CarFitProject.Models.CarListing", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int?>("CarId")
                        .HasColumnType("int")
                        .HasColumnName("car_id");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime2")
                        .HasColumnName("created_at")
                        .HasDefaultValueSql("(getutcdate())");

                    b.Property<bool?>("InstallmentOption")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false)
                        .HasColumnName("installment_option");

                    b.Property<decimal?>("ListingPrice")
                        .HasColumnType("decimal(18, 2)")
                        .HasColumnName("listing_price");

                    b.Property<string>("PaymentMethodAllowed")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("payment_method_allowed");

                    b.Property<string>("RejectionReason")
                        .HasMaxLength(1000)
                        .HasColumnType("nvarchar(1000)")
                        .HasColumnName("rejection_reason");

                    b.Property<int?>("SellerId")
                        .HasColumnType("int")
                        .HasColumnName("seller_id");

                    b.Property<string>("Status")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(40)
                        .HasColumnType("nvarchar(40)")
                        .HasDefaultValue("Draft")
                        .HasColumnName("status");

                    b.HasKey("Id")
                        .HasName("PK__CarListi__3213E83F8D2FA73D");

                    b.HasIndex("CarId");

                    b.HasIndex("SellerId");

                    b.HasIndex(new[] { "Status" }, "IX_CarListings_Status");

                    b.ToTable("CarListings");
                });

            modelBuilder.Entity("CarFitProject.Models.ContactMessage", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("Message")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<string>("Subject")
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.HasKey("Id");

                    b.ToTable("ContactMessages");
                });

            modelBuilder.Entity("CarFitProject.Models.InspectionBooking", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int?>("CarListingId")
                        .HasColumnType("int");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime2")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("CustomerEmail")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("CustomerName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<int?>("MechanicId")
                        .HasColumnType("int");

                    b.Property<string>("PackageType")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("PreferredDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Status")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)")
                        .HasDefaultValue("Pending");

                    b.Property<DateTime?>("StatusUpdatedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("VehicleNotes")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.HasKey("Id");

                    b.HasIndex("CarListingId");

                    b.HasIndex("MechanicId");

                    b.ToTable("InspectionBookings");
                });

            modelBuilder.Entity("CarFitProject.Models.InspectionReport", b =>
                {
                    b.Property<int>("CarId")
                        .HasColumnType("int")
                        .HasColumnName("car_id");

                    b.Property<string>("BodyCondition")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)")
                        .HasColumnName("body_condition");

                    b.Property<decimal?>("CalculatedTrustScore")
                        .HasColumnType("decimal(3, 2)")
                        .HasColumnName("calculated_trust_score");

                    b.Property<bool?>("CarseerAttached")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false)
                        .HasColumnName("carseer_attached");

                    b.Property<string>("CenterName")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("center_name");

                    b.Property<string>("Chassis1Status")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasColumnName("chassis_1_status");

                    b.Property<string>("Chassis2Status")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasColumnName("chassis_2_status");

                    b.Property<string>("Chassis3Status")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasColumnName("chassis_3_status");

                    b.Property<string>("Chassis4Status")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasColumnName("chassis_4_status");

                    b.Property<string>("DescriptionScore")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)")
                        .HasColumnName("description_score");

                    b.Property<int?>("EngineHealthPercent")
                        .HasColumnType("int")
                        .HasColumnName("engine_health_percent");

                    b.Property<bool?>("EngineSmoke")
                        .HasColumnType("bit")
                        .HasColumnName("engine_smoke");

                    b.Property<string>("GearboxStatus")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("gearbox_status");

                    b.Property<DateOnly?>("InspectionDate")
                        .HasColumnType("date")
                        .HasColumnName("inspection_date");

                    b.Property<decimal?>("OverallScore")
                        .HasColumnType("decimal(3, 2)")
                        .HasColumnName("overall_score");

                    b.Property<string>("PaintFillerStatus")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("paint_filler_status");

                    b.Property<string>("PaintStatus")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)")
                        .HasColumnName("paint_status");

                    b.Property<string>("RoofCondition")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("roof_condition");

                    b.HasKey("CarId")
                        .HasName("PK__Inspecti__4C9A0DB3B4E9E2BC");

                    b.ToTable("InspectionReports");
                });

            modelBuilder.Entity("CarFitProject.Models.InspectionTermsGlossary", b =>
                {
                    b.Property<string>("Term")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("term");

                    b.Property<string>("ExplanationAr")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("explanation_ar");

                    b.Property<string>("ExplanationEn")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("explanation_en");

                    b.Property<string>("SeverityLevel")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)")
                        .HasColumnName("severity_level");

                    b.HasKey("Term")
                        .HasName("PK__Inspecti__E0F9670E11FE1C38");

                    b.ToTable("InspectionTermsGlossary", (string)null);
                });

            modelBuilder.Entity("CarFitProject.Models.Make", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("LogoUrl")
                        .HasMaxLength(2048)
                        .HasColumnType("nvarchar(2048)")
                        .HasColumnName("logo_url");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasColumnName("name");

                    b.HasKey("Id");

                    b.HasIndex(new[] { "Name" }, "UX_Makes_Name")
                        .IsUnique();

                    b.ToTable("Makes", (string)null);
                });

            modelBuilder.Entity("CarFitProject.Models.Mechanic", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("City")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("city");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("name");

                    b.Property<string>("Phone")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)")
                        .HasColumnName("phone");

                    b.HasKey("Id");

                    b.HasIndex(new[] { "City" }, "IX_Mechanics_City");

                    b.ToTable("Mechanics", (string)null);
                });

            modelBuilder.Entity("CarFitProject.Models.RecommendationLog", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasColumnName("created_at")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("RecommendedCarIds")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("recommended_car_ids");

                    b.Property<decimal?>("Score")
                        .HasColumnType("decimal(5, 2)")
                        .HasColumnName("score");

                    b.Property<string>("UserId")
                        .HasMaxLength(450)
                        .HasColumnType("nvarchar(450)")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("PK__Recommen__3213E83F3C2D9495");

                    b.HasIndex(new[] { "UserId" }, "IX_RecommendationLog_user_id");

                    b.ToTable("RecommendationLog", (string)null);
                });

            modelBuilder.Entity("CarFitProject.Models.SavedResult", b =>
                {
                    b.Property<string>("UserId")
                        .HasMaxLength(450)
                        .HasColumnType("nvarchar(450)")
                        .HasColumnName("user_id");

                    b.Property<int>("CarId")
                        .HasColumnType("int")
                        .HasColumnName("car_id");

                    b.Property<DateTime?>("SavedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasColumnName("saved_at")
                        .HasDefaultValueSql("(getdate())");

                    b.HasKey("UserId", "CarId")
                        .HasName("PK__SavedRes__9D7797D4910E870F");

                    b.HasIndex("CarId");

                    b.ToTable("SavedResults");
                });

            modelBuilder.Entity("CarFitProject.Models.SearchLog", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime2")
                        .HasColumnName("created_at")
                        .HasDefaultValueSql("getutcdate()");

                    b.Property<string>("FiltersJson")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("filters_json");

                    b.Property<string>("Term")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)")
                        .HasColumnName("term");

                    b.Property<string>("UserId")
                        .HasMaxLength(450)
                        .HasColumnType("nvarchar(450)")
                        .HasColumnName("user_id");

                    b.HasKey("Id");

                    b.HasIndex(new[] { "CreatedAt" }, "IX_SearchLog_created_at");

                    b.ToTable("SearchLog", (string)null);
                });

            modelBuilder.Entity("CarFitProject.Models.Seller", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("City")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("city");

                    b.Property<string>("Email")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)")
                        .HasColumnName("email");

                    b.Property<string>("IdentityUserId")
                        .HasMaxLength(450)
                        .HasColumnType("nvarchar(450)")
                        .HasColumnName("identity_user_id");

                    b.Property<bool>("IsApproved")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false)
                        .HasColumnName("is_approved");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("name");

                    b.Property<string>("Neighborhood")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("neighborhood");

                    b.Property<string>("Phone")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)")
                        .HasColumnName("phone");

                    b.Property<string>("Tier")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)")
                        .HasColumnName("tier");

                    b.Property<string>("Type")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)")
                        .HasColumnName("type");

                    b.HasKey("Id")
                        .HasName("PK__Sellers__3213E83F4F76A86D");

                    b.HasIndex(new[] { "IdentityUserId" }, "IX_Sellers_IdentityUserId");

                    b.ToTable("Sellers");
                });

            modelBuilder.Entity("CarFitProject.Models.SellerInspectionUpload", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<int>("CarId")
                        .HasColumnType("int")
                        .HasColumnName("car_id");

                    b.Property<string>("ContentType")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("content_type");

                    b.Property<string>("FileName")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)")
                        .HasColumnName("file_name");

                    b.Property<long>("FileSizeBytes")
                        .HasColumnType("bigint")
                        .HasColumnName("file_size_bytes");

                    b.Property<string>("StoredPath")
                        .IsRequired()
                        .HasMaxLength(2048)
                        .HasColumnType("nvarchar(2048)")
                        .HasColumnName("stored_path");

                    b.Property<DateTime>("UploadedAtUtc")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime2")
                        .HasColumnName("uploaded_at_utc")
                        .HasDefaultValueSql("(getutcdate())");

                    b.Property<string>("UploadedByUserId")
                        .HasMaxLength(450)
                        .HasColumnType("nvarchar(450)")
                        .HasColumnName("uploaded_by_user_id");

                    b.HasKey("Id");

                    b.HasIndex(new[] { "CarId" }, "IX_SellerInspectionUploads_car_id");

                    b.ToTable("SellerInspectionUploads", (string)null);
                });

            modelBuilder.Entity("CarFitProject.Models.Subscriptions.PaymentTransaction", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<decimal>("AmountJod")
                        .HasColumnType("decimal(18, 3)");

                    b.Property<int?>("CarListingId")
                        .HasColumnType("int");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime2")
                        .HasDefaultValueSql("(getutcdate())");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<string>("StripePaymentIntentId")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<int?>("SubscriptionId")
                        .HasColumnType("int");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasMaxLength(450)
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("CarListingId");

                    b.HasIndex("SubscriptionId");

                    b.HasIndex(new[] { "UserId" }, "IX_PaymentTransactions_UserId");

                    b.HasIndex(new[] { "StripePaymentIntentId" }, "UX_PaymentTransactions_StripePaymentIntentId")
                        .IsUnique()
                        .HasFilter("[StripePaymentIntentId] IS NOT NULL");

                    b.ToTable("PaymentTransactions", (string)null);
                });

            modelBuilder.Entity("CarFitProject.Models.Subscriptions.Subscription", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime?>("CancelledAt")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime2")
                        .HasDefaultValueSql("(getutcdate())");

                    b.Property<DateTime?>("CurrentPeriodEnd")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("CurrentPeriodStart")
                        .HasColumnType("datetime2");

                    b.Property<int>("PlanId")
                        .HasColumnType("int");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<string>("StripeSubscriptionId")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<DateTime?>("TrialEndsAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasMaxLength(450)
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("PlanId");

                    b.HasIndex(new[] { "Status" }, "IX_Subscriptions_Status");

                    b.HasIndex(new[] { "UserId" }, "IX_Subscriptions_UserId");

                    b.HasIndex(new[] { "StripeSubscriptionId" }, "UX_Subscriptions_StripeSubscriptionId")
                        .IsUnique()
                        .HasFilter("[StripeSubscriptionId] IS NOT NULL");

                    b.ToTable("Subscriptions", (string)null);
                });

            modelBuilder.Entity("CarFitProject.Models.Subscriptions.SubscriptionPlan", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<decimal>("AmountJod")
                        .HasColumnType("decimal(18, 3)");

                    b.Property<string>("BillingInterval")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("PlanCode")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("StripePriceId")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("TargetRole")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.HasKey("Id");

                    b.HasIndex(new[] { "PlanCode" }, "UX_SubscriptionPlans_PlanCode")
                        .IsUnique();

                    b.ToTable("SubscriptionPlans", (string)null);
                });

            modelBuilder.Entity("CarFitProject.Models.Testimonial", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("CreatedDate")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasColumnName("created_date")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<bool>("IsApproved")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false)
                        .HasColumnName("is_approved");

                    b.Property<int>("Rating")
                        .HasColumnType("int")
                        .HasColumnName("rating");

                    b.Property<string>("ReviewText")
                        .IsRequired()
                        .HasMaxLength(1000)
                        .HasColumnType("nvarchar(1000)")
                        .HasColumnName("review_text");

                    b.Property<string>("ReviewerName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("reviewer_name");

                    b.Property<string>("UserId")
                        .HasMaxLength(450)
                        .HasColumnType("nvarchar(450)")
                        .HasColumnName("user_id");

                    b.HasKey("Id");

                    b.HasIndex(new[] { "IsApproved" }, "IX_Testimonials_is_approved");

                    b.HasIndex(new[] { "UserId" }, "UX_Testimonials_user_id")
                        .IsUnique()
                        .HasFilter("[user_id] IS NOT NULL");

                    b.ToTable("Testimonials", (string)null);
                });

            modelBuilder.Entity("CarFitProject.Models.UserProfile", b =>
                {
                    b.Property<int>("ProfileId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("profile_id");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ProfileId"));

                    b.Property<int?>("Age")
                        .HasColumnType("int")
                        .HasColumnName("age");

                    b.Property<decimal>("BudgetMax")
                        .HasColumnType("decimal(18, 2)")
                        .HasColumnName("budget_max");

                    b.Property<decimal?>("BudgetMin")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("decimal(18, 2)")
                        .HasDefaultValue(0m)
                        .HasColumnName("budget_min");

                    b.Property<string>("ConditionPref")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)")
                        .HasColumnName("condition_pref");

                    b.Property<bool?>("HasKids")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false)
                        .HasColumnName("has_kids");

                    b.Property<int?>("InstallmentMonths")
                        .HasColumnType("int")
                        .HasColumnName("installment_months");

                    b.Property<bool?>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(true)
                        .HasColumnName("is_active");

                    b.Property<int?>("KidsCount")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(0)
                        .HasColumnName("kids_count");

                    b.Property<string>("MaritalStatus")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasColumnName("marital_status");

                    b.Property<string>("PaymentMethod")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasColumnName("payment_method");

                    b.Property<string>("ProfileName")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasDefaultValue("My Fit Profile")
                        .HasColumnName("profile_name");

                    b.Property<string>("Purpose")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("purpose");

                    b.Property<string>("SizePref")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasColumnName("size_pref");

                    b.Property<string>("TransmissionPref")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)")
                        .HasColumnName("transmission_pref");

                    b.Property<string>("TripType")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)")
                        .HasColumnName("trip_type");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasMaxLength(450)
                        .HasColumnType("nvarchar(450)")
                        .HasColumnName("user_id");

                    b.HasKey("ProfileId")
                        .HasName("PK__UserProf__AEBB701F5EA739D8");

                    b.ToTable("UserProfiles");
                });

            modelBuilder.Entity("CarFitProject.Models.VwAvailableCarDetail", b =>
                {
                    b.Property<string>("BodyCondition")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)")
                        .HasColumnName("body_condition");

                    b.Property<string>("BodyType")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("body_type");

                    b.Property<int>("CarId")
                        .HasColumnType("int");

                    b.Property<string>("City")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("city");

                    b.Property<string>("DescriptionScore")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)")
                        .HasColumnName("description_score");

                    b.Property<string>("EngineSize")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("engine_size");

                    b.Property<string>("ExteriorColor")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("exterior_color");

                    b.Property<string>("ExteriorOptions")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("exterior_options");

                    b.Property<string>("FuelType")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)")
                        .HasColumnName("fuel_type");

                    b.Property<string>("Images")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("images");

                    b.Property<string>("InteriorColor")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("interior_color");

                    b.Property<string>("InteriorOptions")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("interior_options");

                    b.Property<string>("Kilometers")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("kilometers");

                    b.Property<int>("ListingId")
                        .HasColumnType("int");

                    b.Property<decimal?>("ListingPrice")
                        .HasColumnType("decimal(18, 2)")
                        .HasColumnName("listing_price");

                    b.Property<string>("Make")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasColumnName("make");

                    b.Property<string>("Model")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasColumnName("model");

                    b.Property<string>("Neighborhood")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("neighborhood");

                    b.Property<string>("PaintStatus")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)")
                        .HasColumnName("paint_status");

                    b.Property<string>("PaymentMethodAllowed")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("payment_method_allowed");

                    b.Property<decimal?>("Price")
                        .HasColumnType("decimal(18, 2)")
                        .HasColumnName("price");

                    b.Property<string>("RegionalSpecs")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("regional_specs");

                    b.Property<int?>("ScrapedId")
                        .HasColumnType("int")
                        .HasColumnName("scraped_id");

                    b.Property<int?>("Seats")
                        .HasColumnType("int")
                        .HasColumnName("seats");

                    b.Property<int>("SellerId")
                        .HasColumnType("int");

                    b.Property<string>("SellerName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("TechnologyOptions")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("technology_options");

                    b.Property<string>("Transmission")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)")
                        .HasColumnName("transmission");

                    b.Property<string>("Trim")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("trim");

                    b.Property<decimal>("TrustScore")
                        .HasColumnType("decimal(3, 2)");

                    b.Property<int>("Year")
                        .HasColumnType("int")
                        .HasColumnName("year");

                    b.ToTable((string)null);

                    b.ToView("vw_AvailableCarDetails", (string)null);
                });

            modelBuilder.Entity("CarFitProject.ViewModel.RecommendedCarViewModel", b =>
                {
                    b.Property<string>("BodyCondition")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("body_condition");

                    b.Property<int>("CarId")
                        .HasColumnType("int")
                        .HasColumnName("car_id");

                    b.Property<string>("City")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("city");

                    b.Property<string>("DescriptionScore")
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("description_score");

                    b.Property<int>("DynamicMatchScore")
                        .HasColumnType("int")
                        .HasColumnName("DynamicMatchScore");

                    b.Property<decimal>("ListingPrice")
                        .HasPrecision(18, 2)
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("listing_price");

                    b.Property<string>("Make")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("make");

                    b.Property<string>("Model")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("model");

                    b.Property<decimal>("TrustScore")
                        .HasPrecision(18, 2)
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("trust_score");

                    b.Property<int>("Year")
                        .HasColumnType("int")
                        .HasColumnName("year");

                    b.ToTable("RecommendedCarMatches");
                });

            modelBuilder.Entity("CarFitProject.Models.CarImage", b =>
                {
                    b.HasOne("CarFitProject.Models.Car", "Car")
                        .WithMany("CarImages")
                        .HasForeignKey("CarId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Car");
                });

            modelBuilder.Entity("CarFitProject.Models.CarListing", b =>
                {
                    b.HasOne("CarFitProject.Models.Car", "Car")
                        .WithMany("CarListings")
                        .HasForeignKey("CarId")
                        .HasConstraintName("FK__CarListin__car_i__44FF419A");

                    b.HasOne("CarFitProject.Models.Seller", "Seller")
                        .WithMany("CarListings")
                        .HasForeignKey("SellerId")
                        .HasConstraintName("FK__CarListin__selle__45F365D3");

                    b.Navigation("Car");

                    b.Navigation("Seller");
                });

            modelBuilder.Entity("CarFitProject.Models.InspectionBooking", b =>
                {
                    b.HasOne("CarFitProject.Models.CarListing", "CarListing")
                        .WithMany()
                        .HasForeignKey("CarListingId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("CarFitProject.Models.Mechanic", "Mechanic")
                        .WithMany("InspectionBookings")
                        .HasForeignKey("MechanicId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("CarListing");

                    b.Navigation("Mechanic");
                });

            modelBuilder.Entity("CarFitProject.Models.InspectionReport", b =>
                {
                    b.HasOne("CarFitProject.Models.Car", "Car")
                        .WithOne("InspectionReport")
                        .HasForeignKey("CarFitProject.Models.InspectionReport", "CarId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK__Inspectio__car_i__49C3F6B7");

                    b.Navigation("Car");
                });

            modelBuilder.Entity("CarFitProject.Models.SavedResult", b =>
                {
                    b.HasOne("CarFitProject.Models.Car", "Car")
                        .WithMany("SavedResults")
                        .HasForeignKey("CarId")
                        .IsRequired()
                        .HasConstraintName("FK__SavedResu__car_i__5070F446");

                    b.Navigation("Car");
                });

            modelBuilder.Entity("CarFitProject.Models.SellerInspectionUpload", b =>
                {
                    b.HasOne("CarFitProject.Models.Car", "Car")
                        .WithMany("SellerInspectionUploads")
                        .HasForeignKey("CarId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Car");
                });

            modelBuilder.Entity("CarFitProject.Models.Subscriptions.PaymentTransaction", b =>
                {
                    b.HasOne("CarFitProject.Models.CarListing", "CarListing")
                        .WithMany()
                        .HasForeignKey("CarListingId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("CarFitProject.Models.Subscriptions.Subscription", "Subscription")
                        .WithMany("Transactions")
                        .HasForeignKey("SubscriptionId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("CarListing");

                    b.Navigation("Subscription");
                });

            modelBuilder.Entity("CarFitProject.Models.Subscriptions.Subscription", b =>
                {
                    b.HasOne("CarFitProject.Models.Subscriptions.SubscriptionPlan", "Plan")
                        .WithMany("Subscriptions")
                        .HasForeignKey("PlanId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Plan");
                });

            modelBuilder.Entity("CarFitProject.Models.Car", b =>
                {
                    b.Navigation("CarImages");

                    b.Navigation("CarListings");

                    b.Navigation("InspectionReport");

                    b.Navigation("SavedResults");

                    b.Navigation("SellerInspectionUploads");
                });

            modelBuilder.Entity("CarFitProject.Models.Mechanic", b =>
                {
                    b.Navigation("InspectionBookings");
                });

            modelBuilder.Entity("CarFitProject.Models.Seller", b =>
                {
                    b.Navigation("CarListings");
                });

            modelBuilder.Entity("CarFitProject.Models.Subscriptions.Subscription", b =>
                {
                    b.Navigation("Transactions");
                });

            modelBuilder.Entity("CarFitProject.Models.Subscriptions.SubscriptionPlan", b =>
                {
                    b.Navigation("Subscriptions");
                });
#pragma warning restore 612, 618
        }
    }
}
